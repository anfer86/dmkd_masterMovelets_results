...
def Approach1(X_train, y_train, X_test, y_test, par_batch_size, par_epochs, par_lr, par_dropout, save_results, dir_path) :
    ...
    if (save_results) :
        ...
        pd.DataFrame(y_test_true_dec,y_test_pred_dec).to_csv(dir_path + "/model/" + 'model_approach1_prediction.csv', header = True)    
...
def Approach2(X_train, y_train, X_test, y_test, par_batch_size, lst_par_epochs, lst_par_lr, par_dropout, save_results, dir_path) :
    ...
    for k in range(0,k) :
        ...
        #    except :
        #        print("Houve um problema para salvar o modelo, mas o treinamento da rede continua.")
        #        pass
        #                
        #    from numpy import argmax
        #    y_test_true_dec = le.inverse_transform(argmax(y_test1, axis = 1))
        ...
        #    pd.DataFrame(y_test_true_dec,y_test_pred_dec).to_csv(dir_path + "/model/" + 'model_approach2_prediction_Step'+str(k+1)+'.csv', header = True)  
        #
        #  ---------------------------------------------------------------------------------
    ...
    if (save_results) :
        ...
        y_test_pred_dec =  le.inverse_transform(argmax( model.predict(X_test) , axis = 1))
        report = classification_report(y_test_true_dec, y_test_pred_dec )
        classification_report_csv(report, dir_path + "/model/" + 'model_approach2_report.csv', "Approach2")
        model.save(dir_path + "/model/" + 'model_approach2.h5')
        pd.DataFrame(history.history).to_csv(dir_path + "/model/" + 'model_approach2_history.csv')
        pd.DataFrame(y_test_true_dec, y_test_pred_dec).to_csv(dir_path + "/model/" + "model_approach2_prediction.csv", header = False)
...
def ApproachRF(X_train, y_train, X_test, y_test, n_trees_set, save_results, dir_path) :
    ...
    for n_tree in n_trees_set:
        classifier = RandomForestClassifier(verbose=0, n_estimators = n_tree, n_jobs = -1, random_state = 1, criterion = 'gini', bootstrap=True, class_weight="balanced")
        ...
        if (save_results) :
            ...
            pd.DataFrame(y_test, y_predicted).to_csv(dir_path + "/model/" + "model_approachRF"+ str(n_tree) +"_prediction.csv", header = False) 
...
def ApproachRFHP(X_train, y_train, X_test, y_test, save_results, dir_path) :
    ...
    rf_random = RandomizedSearchCV(estimator = rf, param_distributions = random_grid, n_iter = 5, cv = 2, verbose=2, random_state=1, n_jobs = -1)
    ...
    if (save_results) :
        if not os.path.exists(dir_path + "/model/"):
            os.makedirs(dir_path + "/model/")
        report = classification_report(y_test, classifier.predict(X_test) )
        classification_report_csv(report, dir_path + "/model/" + "model_approachRFHP_report.csv","RFHP") 
        pd.DataFrame(line).to_csv(dir_path + "/model/" + "model_approachRFHP_history.csv") 
        pd.DataFrame(y_test, y_predicted).to_csv(dir_path + "/model/" + "model_approachRFHP_prediction.csv", header = False) 
...
def ApproachSVC(X_train, y_train, X_test, y_test, save_results, dir_path) :
    ...
    if (save_results) :
        if not os.path.exists(dir_path + "/model/"):
            os.makedirs(dir_path + "/model/")
        report = classification_report(y_test, classifier.predict(X_test) )
        classification_report_csv(report, dir_path + "/model/" + "model_approachSVC_report.csv","SVC") 
        pd.DataFrame(line).to_csv(dir_path + "/model/" + "model_approachSVC_history.csv") 
# ----------------------------------------------------------------------------------
...
def ApproachMLP(X_train, y_train, X_test, y_test, par_batch_size, par_epochs, par_lr, par_dropout, save_results, dir_path) :
    ...
    if (save_results) :
        ...
        pd.DataFrame(y_test_true_dec, y_test_pred_dec).to_csv(dir_path + "/model/" + "model_MLP_prediction.csv", header = False)    
